...
def calculate_policy_priority(code, title, last_reviewed):
    ...
    high_risk_keywords = [
        'nutrition', 'meal', 'wellness', 'discrimination', 'harassment',
        'bullying', 'safety', 'Title IX', 'special education', 'discipline'
    ]
                ...
                if last_reviewed:
                    try:
                        ...
                        if review_date.year >= 2024:
                            print(f"Skipping {code} - Recently updated ({last_reviewed})")
                            continue
                    except:
                        pass
...
def batch_check_compliance(max_policies=None, output_dir='compliance_reports'):
    ...
    for p in policies[:10]:
        ...
        if p['last_reviewed'] and p['last_reviewed'] != 'Unknown':
            try:
                ...
                pass
        print(f"  {p['code']}: {p['title']} (Last reviewed: {p['last_reviewed']} - {age} ago, Priority: {p['priority']:.1f})")
    ...
    for idx, policy in enumerate(policies):
        ...
        try:
            # Parse policy
            policy_xml, code, title, last_reviewed = parse_policy_to_xml(policy['file_path'])
            ...
            material_issues = [i for i in report_data.get('issues', []) 
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Batch compliance checking for RCSD policies')
    parser.add_argument('--max', type=int, help='Maximum number of policies to check')
    parser.add_argument('--output', default='compliance_reports', help='Output directory')
