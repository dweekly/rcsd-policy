...
class PolicyComplianceChecker:
    ...
    def identify_policy_category(self, policy_title: str, policy_content: str) -> List[str]:
    ...
    def check_ed_code_compliance(self, policy_content: str, categories: List[str]) -> Dict:
        ...
        for category in categories:
            if category in self.ed_code_requirements:
                ...
                for requirement in cat_requirements['key_requirements']:
                    ...
                    if not any(keyword in policy_content.lower() for keyword in req_keywords[:3]):
    ...
    def check_csba_best_practices(self, policy_content: str) -> Dict:
        ...
        if not re.search(r'(Purpose|Intent|Philosophy)', policy_content, re.IGNORECASE):
            best_practice_gaps.append({
                'area': 'policy_structure',
                'gap': 'Missing clear purpose statement'
            })
        
        if not re.search(r'(Legal Reference|Legal|Ref\.?:|Authority)', policy_content, re.IGNORECASE):
            best_practice_gaps.append({
                'area': 'policy_structure',
                'gap': 'Missing legal references section'
            })
        
        # Check for equity considerations
        equity_keywords = ['equity', 'equitable', 'inclusive', 'all students', 'regardless of']
        ...
        monitoring_keywords = ['monitor', 'review', 'evaluate', 'report', 'data', 'measure']
        if not any(keyword in policy_content.lower() for keyword in monitoring_keywords):
            best_practice_gaps.append({
                'area': 'accountability',
        ...
        return {
            'best_practice_gaps': best_practice_gaps
        }
    
    def determine_material_noncompliance(self, compliance_issues: List[Dict], 
                                       best_practice_gaps: List[Dict]) -> Tuple[bool, List[str]]:
        ...
        for issue in compliance_issues:
            if issue['severity'] == 'material':
                material_issues.append(f"Ed Code: {issue['requirement']} - {issue.get('details', '')}")
        ...
        for cat, count in category_issues.items():
            if count >= 3:
                material_issues.append(f"Multiple compliance gaps in {cat} requirements")
    ...
    def analyze_policy(self, policy: Dict) -> Dict:
        ...
        is_material, material_issues = self.determine_material_noncompliance(
            ed_code_results['compliance_issues'],
            csba_results['best_practice_gaps']
        )
    ...
    def analyze_all_policies(self, policies_file: str = "rcsd_policies.json") -> List[Dict]:
        ...
        for i, policy in enumerate(policies):
            print(f"Analyzing policy {i+1}/{len(policies)}: {policy.get('title', 'Unknown')}")
    ...
    def generate_compliance_report(self, results: List[Dict], 
    ...
    def create_excel_summary(self, material_issues: List[Dict], 
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print(f"Material non-compliance summary: material_noncompliance_summary.xlsx")