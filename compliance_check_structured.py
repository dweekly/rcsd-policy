...
{policy_xml}
    ...
    try:
        ...
        return f"Error calling Claude API: {e!s}"
    ...
    try:
        ...
        for issue in root.findall(".//issue"):
            issue_data = {
                ...
                "legal_basis": {"california": [], "federal": []},
                "required_language": [],
                "sample_policies": [],
            }
            ...
            for ca_code in issue.findall(".//california_code"):
                issue_data["legal_basis"]["california"].append(
                    {
                        "citation": ca_code.find("citation").text
                        if ca_code.find("citation") is not None
                        else "",
                        "text": ca_code.find("text").text
                        if ca_code.find("text") is not None
                        else "",
                        "effective_date": ca_code.find("effective_date").text
                        if ca_code.find("effective_date") is not None
                        else "",
                    }
                )
            ...
            for fed_code in issue.findall(".//federal_code"):
                issue_data["legal_basis"]["federal"].append(
                    {
                        "citation": fed_code.find("citation").text
                        if fed_code.find("citation") is not None
                        else "",
                        "text": fed_code.find("text").text
                        if fed_code.find("text") is not None
                        else "",
                    }
                )
            ...
            for req_lang in issue.findall(".//required_language"):
                issue_data["required_language"].append(
                    {
                        "source": req_lang.find("source").text
                        if req_lang.find("source") is not None
                        else "",
                        "text": req_lang.find("text").text
                        if req_lang.find("text") is not None
                        else "",
                    }
                )
            ...
            report["actions"].append(
                {
                    ...
                    "location": action.find("location").text
                    if action.find("location") is not None
                    else "",
                }
            )
        ...
        return {"error": f"Failed to parse XML: {e!s}", "raw": xml_response}
...
def generate_update_report(policy_code, policy_title, report_data):
    ...
    # Material Issues
    material_issues = [
        i for i in report_data.get("issues", []) if i.get("priority") == "MATERIAL"
    ]
    if material_issues:
        ...
        for idx, issue in enumerate(material_issues, 1):
            output += (
                f"{idx}. {issue['title']} (Confidence: {issue['confidence']}%)\n\n"
            )
    ...
    if report_data.get("actions"):
        ...
        for action in sorted(
            report_data["actions"], key=lambda x: x.get("priority", "999")
        ):
