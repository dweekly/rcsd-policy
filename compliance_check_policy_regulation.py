...
class PolicyRegulationChecker(ComplianceChecker):
    ...
    def extract_regulation_provisions(self, reg_path, search_terms):
        ...
        try:
            ...
            provisions.append(
                {"type": "metadata", "title": title, "last_reviewed": last_reviewed}
            )

            # Search for each term
            for term in search_terms:
                pattern = re.compile(
                    r"(.{0,100}" + re.escape(term) + r".{0,200})",
                ...
                for match in matches[:3]:  # Limit to first 3 matches
                    ...
                    provisions.append(
                        {"type": "provision", "term": term, "text": clean_match}
                    )
    ...
    def check_compliance_with_regulation(
        self,
        policy_xml,
        policy_code,
        policy_title,
        ...
        cache_key = self.get_cache_key(
            policy_code + "_with_reg", policy_xml, policy_reviewed
        )
        ...
        if regulation_provisions:
            reg_meta = next(
                (p for p in regulation_provisions if p["type"] == "metadata"), None
            )
            if reg_meta:
                reg_context = (
...
{policy_xml}
        ...
        print("  Calling API with regulation context...")
    ...
    def process_policy(self, file_path):
        ...
        if reg_path:
            ...
            regulation_provisions = self.extract_regulation_provisions(
                reg_path, search_terms
            )
            print(
                f"  Extracted {len([p for p in regulation_provisions if p['type'] == 'provision'])} key provisions from regulation"
            )
        else:
            print("  No matching regulation found")
        ...
        for issue in compliance_data.get("issues", []):
            if (
                issue.get("priority") == "MATERIAL"
                and issue.get("regulation_status") != "COVERED"
            ):
                material_count += 1
