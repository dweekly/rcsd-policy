...
class PolicyResearcher:
    ...
    def __init__(self):
        ...
        self.federal_patterns = {
            ...
            'idea': re.compile(r'IDEA|Individuals with Disabilities Education Act', re.IGNORECASE),
            'ferpa': re.compile(r'FERPA|Family Educational Rights and Privacy Act', re.IGNORECASE),
        ...
        self.compliance_areas = {
            '0000-0999': ['Board Governance', 'District Organization', 'School Accountability'],
            '1000-1999': ['Community Relations', 'Parent Involvement', 'Complaints'],
            '2000-2999': ['Administration', 'Superintendent', 'Management Systems'],
            '3000-3999': ['Business Operations', 'Facilities', 'Safety', 'Transportation'],
    ...
    def research_policies(self, policies_file: str = 'extracted_policies.json') -> List[PolicyResearch]:
    ...
    def _research_single_policy(self, policy: Dict) -> Optional[PolicyResearch]:
        ...
        recommendations = self._generate_recommendations(code, content, potential_issues)
    ...
    def _check_outdated_references(self, code: str, content: str) -> Optional[str]:
        ...
        if outdated_years:
            return f"Contains potentially outdated references from {min(outdated_years)}"
    ...
    def _check_missing_mandates(self, code: str, content: str) -> Optional[str]:
        ...
        if code.startswith('5141') and 'health' in content.lower():
            if 'medication' not in content.lower():
                return "Health policy may be missing medication administration procedures"
    ...
    def _check_procedural_gaps(self, code: str, content: str) -> Optional[str]:
        ...
        has_procedures = any(keyword in content.lower() for keyword in procedure_keywords)
    ...
    def _check_notification_requirements(self, code: str, content: str) -> Optional[str]:
    ...
    def _generate_recommendations(self, code: str, content: str, issues: List[str]) -> List[str]:
        """Generate recommendations based on findings"""
        recommendations = []
        
        if issues:
            recommendations.append("Review and update policy to address identified compliance issues")
        ...
        if 'adopted' in content.lower():
            date_pattern = re.compile(r'adopted.*?(\d{1,2}/\d{1,2}/\d{4})', re.IGNORECASE)
            ...
            if match:
                ...
                try:
                    ...
                    if (datetime.now() - adopted_date).days > 1825:  # 5 years
                        recommendations.append("Policy is over 5 years old - consider comprehensive review")
                except:
                    pass
        
        # Specific recommendations by policy area
        if code.startswith('3'):
            recommendations.append("Ensure business procedures align with current fiscal requirements")
        elif code.startswith('5'):
            recommendations.append("Verify student policies comply with current due process requirements")
        elif code.startswith('6'):
            recommendations.append("Check instructional policies against current state standards")
        
        return recommendations
    
    def _check_csba_alignment(self, code: str, content: str) -> str:
        """Check alignment with CSBA best practices"""
        csba_indicators = ['csba', 'california school boards association', 'best practice']
        ...
        elements = {
            'purpose': any(word in content.lower() for word in ['purpose', 'intent', 'goal']),
            'definitions': 'definition' in content.lower() or 'means' in content.lower(),
            'procedures': 'procedure' in content.lower() or 'process' in content.lower(),
            'responsibilities': 'responsib' in content.lower() or 'shall' in content.lower()
        }
    ...
    def export_research_summary(self, output_file: str = 'research_summary.xlsx') -> None:
        ...
        for research in self.research_results:
            summary_data.append({
                ...
                'Issue Details': '; '.join(research.potential_issues) if research.potential_issues else 'None',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Research extracted policies for compliance')
    parser.add_argument('--input', default='extracted_policies.json', help='Input JSON file with extracted policies')
    parser.add_argument('--output', default='policy_research.json', help='Output JSON file for research results')
    parser.add_argument('--excel', action='store_true', help='Also export Excel summary')
